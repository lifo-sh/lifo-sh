...
export function createChildProcess(executeCapture?: ExecuteCapture) {
  ...
  function execSync(): never {
    throw new Error('child_process.execSync() is not supported in BrowserOS');
  }

  function spawn(): never {
    throw new Error('child_process.spawn() is not supported in BrowserOS');
  }

  function fork(): never {
    throw new Error('child_process.fork() is not supported in BrowserOS');
  }
