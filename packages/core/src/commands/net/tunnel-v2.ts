 ...
 *
 * Usage:
 *   tunnel [--server=ws://localhost:3001] [-v]
 *
 * Access from host:
 *   http://localhost:3001/3000/ → Port 3000 inside Lifo
 */
...
Options:
  --server <url>    Tunnel server URL (default: ws://localhost:3001)
...
Example:
  ...
  curl http://localhost:3001/3000/
  open http://localhost:3001/3000/

Access tunneled servers using path-based routing:
  http://localhost:3001/3000/         → Port 3000 inside Lifo
  http://localhost:3001/8080/api      → Port 8080, path /api
  http://localhost:3001/3000/users    → Port 3000, path /users\n`);
      return 0;
    }
    ...
    try {
      ...
      if (ports.length === 0) {
        ...
        for (const port of ports) {
          const httpUrl = server.replace('ws://', 'http://').replace('wss://', 'https://').replace(/:\d+$/, ':3001');
