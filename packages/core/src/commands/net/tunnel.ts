...
function createTunnelImpl(portRegistry?: Map<number, VirtualRequestHandler>): Command {
  return async (ctx) => {
    ...
    if ('help' in options && options.help) {
      ...
      ctx.stdout.write('  tunnel --server=ws://example.com:3001 Custom tunnel server\n\n');
      ...
      ctx.stdout.write('  http://localhost:3001/ → Port 5173, path /\n');
      ctx.stdout.write('  http://localhost:3001/src/main.ts → Port 5173, path /src/main.ts\n');
      return 0;
    }
    ...
    function logActivePorts() {
      ...
      for (const port of ports) {
        if (defaultPort) {
          ctx.stdout.write(`  - Port ${port}: http://localhost:3001/\n`);
        } else {
